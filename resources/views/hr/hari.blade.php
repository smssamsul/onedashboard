...
@section('title', 'Hari')
@section('page_title', 'Manajemen Hari')
...
<script>
        ...
        tbody.innerHTML = data.map((item, index) => {
            ...
            return `
                <tr>
                    ...
                    <td>
                        ${item.status === 'pending' ? `
                            <button class="btn btn-success" onclick="approveHari(${item.id})" style="padding: 0.375rem 0.75rem; font-size: 0.75rem; margin-right: 0.5rem;">Setujui</button>
                            <button class="btn btn-danger" onclick="rejectHari(${item.id})" style="padding: 0.375rem 0.75rem; font-size: 0.75rem;">Tolak</button>
                        ` : '-'}
                    </td>
                </tr>
            `;
    ...
    async function approveHari(id) {
        if (!confirm('Apakah Anda yakin ingin menyetujui pengajuan hari ini?')) {
            return;
        }

        try {
            const token = localStorage.getItem('auth_token') || sessionStorage.getItem('auth_token');
            const response = await fetch(`/api/hr/hari/${id}/approve`, {
                method: 'POST',
                headers: {
                    'Authorization': 'Bearer ' + token,
                    'Accept': 'application/json'
            ...
            if (result.success) {
                ...
                alert(result.message || 'Pengajuan hari berhasil disetujui');
            } else {
                alert(result.message || 'Terjadi kesalahan');
            }
        } catch (error) {
            console.error('Error approving hari:', error);
            alert('Terjadi kesalahan saat menyetujui pengajuan');
        }
    }

    async function rejectHari(id) {
        ...
        try {
            const token = localStorage.getItem('auth_token') || sessionStorage.getItem('auth_token');
            const response = await fetch(`/api/hr/hari/${id}/reject`, {
                method: 'POST',
                headers: {
                    'Authorization': 'Bearer ' + token,
                    'Accept': 'application/json',
            ...
            if (result.success) {
                loadData(currentPage);
                alert(result.message || 'Pengajuan hari berhasil ditolak');
            } else {
                alert(result.message || 'Terjadi kesalahan');
            }
        } catch (error) {
            console.error('Error rejecting hari:', error);
