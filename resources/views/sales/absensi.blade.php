...
<script>
    ...
    // Load absensi data - menggunakan endpoint by-current-user
    ...
    function renderTable(data, pagination = null) {
        ...
        tbody.innerHTML = data.map((item, index) => {
            ...
            // Tombol check out hanya muncul jika sudah check in tapi belum check out
            const checkOutButton = item.check_in && !item.check_out ? 
                `<button class="btn btn-primary" onclick="openCheckOutModalFromAbsensi(${item.id})" style="padding: 0.375rem 0.75rem; font-size: 0.75rem;">Check Out</button>` : '-';
