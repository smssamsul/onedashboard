...
@section('content')
    ...
    <div class="page-header">
        <div>
            <h1>Kelola Leads</h1>
            <p>Kelola dan pantau semua leads dari customer</p>
        </div>
        <div style="display: flex; gap: 0.75rem;">
            ...
            <button class="btn btn-primary" onclick="openGenerateModal()" style="background: white; color: var(--primary);">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <line x1="12" y1="5" x2="12" y2="19"/>
                    <line x1="5" y1="12" x2="19" y2="12"/>
                </svg>
    ...
    <div class="modal-overlay" id="generateModal">
        <div class="modal">
            <div class="modal-header">
                <h3>Generate Leads dari Customer</h3>
            ...
            <div class="modal-body">
                <div class="info-box">
                    ...
                    <span>Generate leads otomatis berdasarkan data customer. Customer yang sudah memiliki lead dengan label yang sama akan di-skip.</span>
                </div>
                <div class="form-group">
                    <label>Label Lead *</label>
                    <input type="text" id="genLeadLabel" placeholder="Contoh: Promo Akhir Tahun 2024" required />
                </div>
                <div class="form-group">
                    <label>Assign Sales *</label>
                    <select id="genSalesId" required>
                        <option value="">Pilih Sales</option>
                    </select>
                </div>
                <div class="form-group">
                    <div class="checkbox-group" onclick="document.getElementById('genAllCustomers').click()">
                        <input type="checkbox" id="genAllCustomers" onchange="toggleFilters()" onclick="event.stopPropagation()" />
                        <label for="genAllCustomers">Generate dari Semua Customer</label>
                    </div>
                </div>
                <div id="filterOptions">
                    <div class="form-group">
                        <label>Filter Pendapatan Minimum</label>
                        <select id="genMinSalary">
                    ...
                    <div class="form-group">
                        <label>Filter Riwayat Order</label>
                        <select id="genStatusOrder">
                            <option value="">Tidak ada filter</option>
                        </select>
                    </div>
                </div>
            ...
            <div class="modal-footer">
                <button class="btn btn-secondary" onclick="closeGenerateModal()">Batal</button>
                <button class="btn btn-primary" onclick="generateLeads()" id="btnGenerate">Generate Leads</button>
            </div>
        </div>
    </div>
    ...
    <div class="modal-overlay" id="broadcastModal">
        <div class="modal" style="max-width: 600px;">
            ...
            <div class="modal-body">
                ...
                <div class="form-group" id="broadcastSalesFilterGroup">
                    <label>Filter Assign Sales</label>
                    <select id="broadcastSalesFilter">
...
<script>
    ...
    function openBroadcastModal() {
        ...
        document.getElementById('broadcastSalesFilter').value = '';
        
        // Load labels and sales for filter
        loadBroadcastLabels();
        loadBroadcastSales();
    }
    ...
    async function loadBroadcastSales() {
        try {
            const response = await fetch(`${API_BASE}/lead/sales-list`, { headers: getHeaders() });
            const result = await response.json();
            if (result.success && result.data) {
                const select = document.getElementById('broadcastSalesFilter');
                select.innerHTML = '<option value="">Semua Sales</option>';
                result.data.forEach(sales => {
                    const option = document.createElement('option');
                    option.value = sales.id;
                    const levelText = sales.level === '1' ? ' (Head Sales)' : ' (Sales)';
                    option.textContent = `${sales.nama}${levelText}`;
                    select.appendChild(option);
                });
            }
        } catch (error) {
            console.error('Error loading broadcast sales:', error);
        }
    }
    ...
    async function sendBroadcast() {
        ...
        const salesId = document.getElementById('broadcastSalesFilter').value;
    ...
    async function loadStatistics() {
        try {
            const response = await fetch(`${API_BASE}/lead/statistics`, {
                headers: getHeaders()
            });
            const result = await response.json();
            if (result.success) {
                document.getElementById('stat-total').textContent = result.data.total_leads || 0;
                document.getElementById('stat-new').textContent = result.data.new_leads || 0;
                document.getElementById('stat-contacted').textContent = result.data.contacted_leads || 0;
                document.getElementById('stat-converted').textContent = result.data.converted_leads || 0;
            }
        } catch (error) {
    ...
    async function loadLeads(page = 1) {
        ...
        let url = `${API_BASE}/lead?page=${page}&per_page=15`;
                    ...
                    <tr>
                        <td colspan="8">
                            <div class="empty-state">
                                ...
                                <p>Generate leads pertama Anda dari data customer</p>
                                <button class="btn btn-primary" onclick="openGenerateModal()">Generate Leads Sekarang</button>
                            </div>
                        </td>
                    </tr>
                `;
    ...
    // Load Products for Generate Modal
    async function loadStatusOrders() {
        try {
            const response = await fetch(`${API_BASE}/lead/status-orders-list`, { headers: getHeaders() });
            const result = await response.json();
            if (result.success) {
                const select = document.getElementById('genStatusOrder');
                result.data.forEach(statusOrder => {
                    const option = document.createElement('option');
                    option.value = statusOrder;
                    option.textContent = statusOrder;
                    select.appendChild(option);
                });
            }
        } catch (error) {
    ...
    async function loadSalesForCreate() {
        try {
            const response = await fetch(`${API_BASE}/lead/sales-list`, { headers: getHeaders() });
            const result = await response.json();
            if (result.success && result.data) {
                salesListForCreate = result.data;
                const select = document.getElementById('createSalesId');
                select.innerHTML = '<option value="">Pilih Sales (opsional)</option>';
                salesListForCreate.forEach(sales => {
                    const option = document.createElement('option');
                    option.value = sales.id;
                    const levelText = sales.level === '1' ? ' (Head Sales)' : ' (Sales)';
                    option.textContent = `${sales.nama}${levelText}`;
    ...
    function openCreateModal() {
        ...
        document.getElementById('createCustomerDropdown').style.display = 'none';
        document.getElementById('createSalesId').value = '';
    ...
    async function createLead() {
        ...
        const salesId = document.getElementById('createSalesId').value;
        const label = document.getElementById('createLeadLabel').value.trim();
    ...
    async function loadSalesForGenerate() {
        try {
            const response = await fetch(`${API_BASE}/lead/sales-list`, { headers: getHeaders() });
            const result = await response.json();
            if (result.success && result.data) {
                salesList = result.data;
                const select = document.getElementById('genSalesId');
    ...
    function openGenerateModal() {
        ...
        document.getElementById('genLeadLabel').value = '';
        document.getElementById('genSalesId').value = '';
        document.getElementById('genAllCustomers').checked = false;
        document.getElementById('genMinSalary').value = '';
        document.getElementById('genStatusOrder').value = '';
        document.getElementById('filterOptions').style.display = 'block';
        loadSalesForGenerate();
    }
    ...
    function toggleFilters() {
        const allChecked = document.getElementById('genAllCustomers').checked;
        const filterOptions = document.getElementById('filterOptions');
        filterOptions.style.display = allChecked ? 'none' : 'block';
        if (allChecked) {
            document.getElementById('genMinSalary').value = '';
            document.getElementById('genStatusOrder').value = '';
        }
    ...
    async function generateLeads() {
        const label = document.getElementById('genLeadLabel').value.trim();
        const salesId = document.getElementById('genSalesId').value;
        const allCustomers = document.getElementById('genAllCustomers').checked;
        const minSalary = document.getElementById('genMinSalary').value;
        const statusOrder = document.getElementById('genStatusOrder').value;
        ...
        if (!salesId) {
            showAlert('Assign Sales wajib dipilih', 'error');
            return;
        }

        if (!allCustomers && !minSalary && !statusOrder) {
            showAlert('Pilih "Semua Customer" atau minimal satu filter', 'error');
            return;
        }
        ...
        try {
            ...
            if (allCustomers) {
                ...
                if (minSalary) body.min_salary = parseInt(minSalary) * 1000000;
                if (statusOrder) body.status_order = statusOrder;
            }

            const response = await fetch(`${API_BASE}/lead/generate`, {
                method: 'POST',
                headers: getHeaders(),
                body: JSON.stringify(body)
            });
            ...
            if (result.success) {
                closeGenerateModal();
                showAlert(`Berhasil! ${result.data.leads_created} leads dibuat, ${result.data.leads_skipped} di-skip.`, 'success');
                loadLeads();
                loadStatistics();
            } else {
                showAlert(result.message || 'Gagal generate leads', 'error');
            }
        } catch (error) {
            console.error('Error generating leads:', error);
            showAlert('Terjadi kesalahan saat generate leads', 'error');
        } finally {
            btn.disabled = false;
    ...
    function copyPhoneNumber(phoneNumber) {
        ...
        navigator.clipboard.writeText(cleanNumber).then(() => {
            showAlert('Nomor WA berhasil disalin: ' + cleanNumber, 'success');
        }).catch(err => {
            ...
            try {
                ...
                showAlert('Gagal menyalin nomor', 'error');
            }
