...
@section('page_title', 'Manajemen Izin Telat')
...
<script>
    ...
    async function loadData(page = 1) {
        try {
            ...
            let url = `/api/hr/izin-telat?page=${page}`;
            if (filterKaryawan) {
        ...
        tbody.innerHTML = data.map((item, index) => {
            ...
            return `
                <tr>
                    ...
                    <td>
                        ${item.status === 'pending' ? `
                            <button class="btn btn-success" onclick="approveIzinTelat(${item.id})" style="padding: 0.375rem 0.75rem; font-size: 0.75rem; margin-right: 0.5rem;">Setujui</button>
                            <button class="btn btn-danger" onclick="rejectIzinTelat(${item.id})" style="padding: 0.375rem 0.75rem; font-size: 0.75rem;">Tolak</button>
                        ` : '-'}
                    </td>
                </tr>
            `;
    ...
    async function approveIzinTelat(id) {
        if (!confirm('Apakah Anda yakin ingin menyetujui pengajuan izin telat ini?')) {
            return;
        }

        try {
            const token = localStorage.getItem('auth_token') || sessionStorage.getItem('auth_token');
            const response = await fetch(`/api/hr/izin-telat/${id}/approve`, {
                method: 'POST',
                headers: {
                    'Authorization': 'Bearer ' + token,
                    'Accept': 'application/json'
            ...
            if (result.success) {
                ...
                alert(result.message || 'Pengajuan izin telat berhasil disetujui');
            } else {
                alert(result.message || 'Terjadi kesalahan');
            }
        } catch (error) {
            console.error('Error approving izin telat:', error);
            alert('Terjadi kesalahan saat menyetujui pengajuan');
        }
    }

    async function rejectIzinTelat(id) {
        ...
        try {
            const token = localStorage.getItem('auth_token') || sessionStorage.getItem('auth_token');
            const response = await fetch(`/api/hr/izin-telat/${id}/reject`, {
                method: 'POST',
                headers: {
                    'Authorization': 'Bearer ' + token,
                    'Accept': 'application/json',
            ...
            if (result.success) {
                loadData(currentPage);
                alert(result.message || 'Pengajuan izin telat berhasil ditolak');
            } else {
                alert(result.message || 'Terjadi kesalahan');
            }
        } catch (error) {
            console.error('Error rejecting izin telat:', error);
