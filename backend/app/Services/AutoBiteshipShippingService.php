...
class AutoBiteshipShippingService
{
    ...
    /**
     * Default origin payload dari config/services.php (sama konsep dengan
     * OrderResiController). Dibaca lewat config(), BUKAN env() langsung -
     * env() di luar file config berhenti baca .env begitu `config:cache`
     * dijalankan (bagian rutin deploy), dan akan selalu jatuh ke fallback
     * hardcode di bawah walau .env sudah diisi benar.
     *
